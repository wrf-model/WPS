...
class OrographyStats:
  ...
  def calc_oa( self ):
    ...
    nu = np.sum( self.box[:, :int(self.box.shape[1] / 2)] > self.mean )
    nd = np.sum( self.box[:, int(self.box.shape[1] / 2):] > self.mean )
    self.oa[0] = ( nu - nd ) / ( nu + nd ) if ( ( nu + nd ) > 0 ) else 0.0

    # oa2 is the orographic asymmetry in the South direction
    nu = np.sum( self.box[:int(self.box.shape[0] / 2), :] > self.mean )
    nd = np.sum( self.box[int(self.box.shape[0] / 2):, :] > self.mean )
    self.oa[1] = ( nu - nd ) / ( nu + nd ) if ( ( nu + nd ) > 0 ) else 0.0

    # Pre-compute the geometric diagonal of the box
    slope = self.box.shape[1] / self.box.shape[0]
    ...
    nd = np.sum( self.box[~upstream] > self.mean )
  ...
  def calc_ol( self ):
    ...
    self.ol[0] = np.sum( interior > self.hc ) / interior.size
    ...
    self.ol[1] = np.sum( interior > self.hc ) / interior.size
    ...
    self.ol[2] = ( np.sum( interiorA > self.hc ) + np.sum( interiorB > self.hc ) ) / ( interiorA.size + interiorB.size )
    ...
    self.ol[3] = ( np.sum( interiorA > self.hc ) + np.sum( interiorB > self.hc ) ) / ( interiorA.size + interiorB.size )
