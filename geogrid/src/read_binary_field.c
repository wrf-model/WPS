...
void read_binary_field(float * data, int * in_size, int * endian, int * nitems, char * fname, int * len, int * is_signed, int * status)
{
   FILE * fp;
   unsigned char * c;
   int cnt, i, j, k;
   ...
   else if (*in_size == 3)
   {
      ...
      {
         for(i=(*nitems)-1; i>=0; i--)
            data[i] = (float)((c[3*i+2]<<16)|(c[3*i+1]<<8)|(c[3*i]));
      }
      else
      {
         for(i=(*nitems)-1; i>=0; i--)
   ...
   else if (*in_size == 4)
   {
      ...
      {
         for(i=(*nitems)-1; i>=0; i--)
            data[i] = (float)((c[4*i+3]<<24)|(c[4*i+2]<<16)|(c[4*i+1]<<8)|c[4*i]);
      }
      else
      {
         for(i=(*nitems)-1; i>=0; i--)
