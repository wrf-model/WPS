...
module source_data_module
   ...
   subroutine get_datalist()
    ...
    20 rewind(funit)
      ...
      allocate(is_landmask_water(num_entries))
      allocate(is_landmask_land(num_entries))
    ...
    30 buffer = ' '
      ...
      ! Are we beginning a new field specification?
         ...
         if (i <= num_entries) then
            is_path(i) = .false.
            is_landmask_water(i) = .false.
            is_landmask_land(i) = .false.
            is_masked(i) = .false.
   ...
   subroutine get_source_params(res_string)
      ...
      ! Local variables
      integer :: idx, i, ispace, eos, iquoted, funit
      ...
      do idx=1,num_entries
         ...
         if (.not. list_search(source_interp_option(idx), ckey=res_string, cvalue=source_interp_string(idx))) then
            ...
            if (list_search(source_interp_option(idx), ckey=temp_str, cvalue=source_interp_string(idx))) then
               ...
               call mprintf(.true., ERROR, 'Could not find default or %s interpolator sequence.'// &
         ...
         if (.not. list_search(source_res_path(idx), ckey=res_string, cvalue=source_path(idx))) then
            ...
            if (list_search(source_res_path(idx), ckey=temp_str, cvalue=source_path(idx))) then
               ...
               call mprintf(.true., ERROR, 'Could not find default or %s data source. '// &
                            'The source specified in namelist.wps is not listed in GEOGRID.TBL.', &
                            s1=trim(res_string))
            end if
         end if
    ...
    1000 call mprintf(.true.,ERROR,'Could not open %s', s1=trim(test_fname))
   ...
   subroutine datalist_destroy()
      ...
      if (associated(source_wordsize)) then
         ...
         deallocate(is_landmask_water)
         deallocate(is_landmask_land)
...
#include "wrf_io_flags.h"
      ...
      if (output_field_state == RETURN_LANDMASK) then
         ...
         call get_landmask_field(field_name, is_water_mask, landmask, istatus)
         if (istatus == 0) then
            temphash(129:256) = ' '
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   subroutine get_landmask_field(landmask_name, is_water_mask, landmask, istatus)
...
!      ry = real(source_tile_y(idx)) * real(floor((ry-0.5*source_dy(idx))/ real(source_tile_y(idx)))) + 1.0
   ...
   function check_data_specification( )
      ...
      do i=1,num_entries
         if (is_landmask_land(i) .or. is_landmask_water(i)) then
      ...
      do i=1,num_entries
         if (is_dest_fieldtype(i)) then
            if (source_dest_fieldtype(i) == CONTINUOUS) then
               if (is_landmask_water(i) .or. is_landmask_land(i)) then
